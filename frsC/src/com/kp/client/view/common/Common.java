...
public class Common {
	...
	private void initListener(){
		...
		fileToolBar.addClickListener(showTransViewListener);//传输视图
	...
	Listener downloadListener = new Listener() {
		@Override
		public void handleEvent(Event arg0) {
			String[] names = fileTable.getSelectionFiles();
	...
	Listener deleteListener = new Listener() {
		@Override
		public void handleEvent(Event arg0) {
			String[] filename = fileTable.getSelectionFiles();  //文件名
			if(filename.length > 0){
				MessageBox dialog=new MessageBox(view.getShell(), SWT.YES|SWT.NO);
				dialog.setText("温馨提示");
				dialog.setMessage("确认要删除“"+filename[0]+"”"+(filename.length>1 ? "等"+filename.length+"个文件" : "")+"吗？");
				if(dialog.open() == 64){//64返回确定，128返回取消
					view.getFileController().deleteFile(filename[0]);
				}
			}
		}
	};
...
*/
	 ...
	 * 传输视图显示事件处理
	 */
	Listener showTransViewListener = new Listener() {
		@Override
		public void handleEvent(Event arg0) {
			view.getViewController().showTransView();
		}
	};
