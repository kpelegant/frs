...
public class Register extends Shell {
	...
	private void init() {
		...
		cPolicy = new CLabel(mainView, SWT.RIGHT);
		cPolicy.setText("策略");
		cPolicy.setBackground(mainBG);
		cPolicy.setFont(cFont);
		tPolicy = new Text2(mainView, SWT.NONE, new RGB(173, 214, 244));
		ePolicy = new CLabel(mainView, SWT.NONE);
		ePolicy.setForeground(redColor);
		ePolicy.setBackground(mainBG);
		tPolicy.addListener(SWT.FocusOut, tPolicyCheckListener);
		...
		nosee.addPaintListener(noseeButtonPaintListener);
	...
	public void setBounds(){
		...
		cPolicy.setBounds(100, 190, 60, 30);
		tPolicy.setBounds(100+80, 190, 200, 30);
		ePolicy.setBounds(100+280, 190, 200, 30);
	 ...
	 * 重绘ButtonView边框
	 */
	PaintListener noseeButtonPaintListener = new PaintListener() {
		@Override
		public void paintControl(PaintEvent arg0) {
			GC gc_container_1 = new GC(nosee);  
            gc_container_1.setLineWidth(1);  
            gc_container_1.drawRoundRectangle(0, 0, nosee.getSize().x-1, nosee.getSize().y-1, 5, 5);  
	...
	Listener registerClickListener = new Listener() {
		public void handleEvent(Event arg0) {
			if(checkTUser()&&checkTPassword(ePassword, tPassword)&&checkTPassword2(ePassword2, tPassword2, tPassword)&&checkTPolicy()&&checkTVert()){
				/*System.out.println("注册用户名:"+tUser.getText()+",注册密码"+tPassword.getText());
				System.out.println(view);*/
				System.out.println("register（c）准备注册");
				view.getUserController().register(tUser.getText(), tPassword.getText(), tPolicy.getText());
	...
	Listener tPassword2CheckListener = new Listener() {
		@Override
		public void handleEvent(Event arg0) {
			if(checkTPassword(ePassword, tPassword)){
	 ...
	 * 检测策略
	 */
	private boolean checkTPolicy(){
		ePolicy.setText("");
		String policy = tPolicy.getText();
		if(policy.compareTo("")==0){
			ePolicy.setText("用户策略不能为空");
			return false;
		}else {
			return true;
		}
	 ...
	 * 策略输入框检测事件处理
	 */
	Listener tPolicyCheckListener = new Listener() {
		@Override
		public void handleEvent(Event arg0) {
			checkTPolicy();
		}
